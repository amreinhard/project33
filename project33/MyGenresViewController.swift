...
//  project33
//
//  Created by Amanda Reinhard on 4/23/18.
    ...
    @objc func saveTapped() {
        ...
        database.fetchAllSubscriptions { [unowned self] subscriptions, error in
            if error == nil {
                if let subscriptions = subscriptions {
                    for subscription in subscriptions {
                        database.delete(withSubscriptionID: subscription.subscriptionID) { str, error in
                            if error != nil {
                                let ac = UIAlertController(title: "Error", message: "There was a problem retrieving your subscriptions: \(error!.localizedDescription)", preferredStyle: .alert)
                    ...
                    for genre in self.myGenres {
                        ...
                        database.save(subscription) { result, error in
                            if let error = error {
                                let ac = UIAlertController(title: "Error", message: "There was a problem saving your subscription: \(error.localizedDescription)", preferredStyle: .alert)
                                ac.addAction(UIAlertAction(title: "OK", style: .default))
                                self.present(ac, animated: true)
                            }
                        }
                ...
                let ac = UIAlertController(title: "Error", message: "You have no subscriptions. Please subscribe to a genre to utilize this feature. \(error!.localizedDescription)", preferredStyle: .alert)
                ac.addAction(UIAlertAction(title: "OK", style: .default))
                self.present(ac, animated: true)
            }
        }
