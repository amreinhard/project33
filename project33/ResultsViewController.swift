...
class ResultsViewController: UITableViewController {
    ...
    override func viewDidLoad() {
        ...
        CKContainer.default().publicCloudDatabase.perform(query, inZoneWith: nil) { [unowned self] results, error in
            if let error = error {
                let ac = UIAlertController(title: "Error", message: "There was a problem submitting your query: \(error.localizedDescription)", preferredStyle: .alert)
        ...
        CKContainer.default().publicCloudDatabase.fetch(withRecordID: whistle.recordID) { [unowned self] record, error in
            if let error = error {
                DispatchQueue.main.async {
                    let ac = UIAlertController(title: "Error", message: "There was a problem downloading this whistle: \(error.localizedDescription)", preferredStyle: .alert)
                    ac.addAction(UIAlertAction(title: "OK", style: .default))
                    self.present(ac, animated: true)
